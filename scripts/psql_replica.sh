...
for name in POSTGRES_HOST POSTGRES_REPLICA_HOST POSTGRES_REPLICA_PORT POSTGRES_LOGICAL_SUBSCRIBER_HOST POSTGRES_UPGRADE_OLD_HOST POSTGRES_UPGRADE_NEW_HOST PGBOUNCER_HOST POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD ALLOW_NONLOCAL_PG ALLOW_SYSTEM_DB PGWORKBENCH_EXPERIMENT_MODE; do
  if [[ ${!name+x} ]]; then
