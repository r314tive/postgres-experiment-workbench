...
restore_spec_overrides() {
  ...
  for ((i = 0; i < ${#PRESERVED_ENV_NAMES[@]}; i++)); do
    ...
    case "$name" in
      PROFILE_SIZE|PROFILE_SECONDS|SQL|WORKLOAD_IMAGE|WORKLOAD_COMMAND|WORKLOAD_CMD|WORKLOAD_RUN_LOG|WORKLOAD_LOG_DIR|WORKLOAD_LOG_FILE|PGBENCH_*|PG_*|NOISIA_DURATION|NOISIA_JOBS|NOISIA_EXTRA_ARGS|NOISIA_WAIT_LOCKTIME_MIN|NOISIA_WAIT_LOCKTIME_MAX|NOISIA_TEMP_FILES_RATE|NOISIA_TEMP_FILES_SCALE_FACTOR|LOGICAL_REPLICATION_*)
