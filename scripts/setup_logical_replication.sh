...
for name in POSTGRES_HOST POSTGRES_REPLICA_HOST POSTGRES_LOGICAL_SUBSCRIBER_HOST POSTGRES_UPGRADE_OLD_HOST POSTGRES_UPGRADE_NEW_HOST PGBOUNCER_HOST POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD ALLOW_NONLOCAL_PG ALLOW_SYSTEM_DB PGWORKBENCH_EXPERIMENT_MODE LOGICAL_REPLICATION_PUBLICATION LOGICAL_REPLICATION_SUBSCRIPTION LOGICAL_REPLICATION_SLOT; do
  if [[ ${!name+x} ]]; then
